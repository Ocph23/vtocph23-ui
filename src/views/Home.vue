...
<template>
  <VTCard class="h-screen" title="Title" subtitle="subtitle" description="description">
    ...
    <div class="w-full p-2 flex justify-end bg-rose-100 rounded-tr-lg rounded-tl-lg">
      <div class="rounded-2xl flex items-center justify-center w-8 h-8 bg-slate-800">
        <VTIconResetPassword color="primary" size="sm" />
      </div>
    </div>
    ...
    >
      <button
        @click="tableData.text = !tableData.text"
        class="my-10 p-3 mr-4 text-2xl text-white capitalize font-medium antialiased 
        rounded-lg bg-rose-400 hover:shadow-md drop-shadow-md hover:bg-pink-600/10
         shadow-inner shadow-white/30 hover:drop-shadow-sm hover:shadow-orange-500/50
          active:bg-pink-600/10 ring-2 ring-yellow-200/20 hover:backdrop-blur-sm
           hover:ring-yellow-200/20 hover:transition-colors hover:duration-300"
      >
        kas muncul barang lucuk!
      </button>
      <transition
        enter-active-class="duration-500 scale-50 transition-all"
        enter-from-class="opacity-0 scale-0"
        enter-to-class="opacity-100 scale-100"
        leave-active-class="duration-500 scale-50 transition-all"
        leave-from-class="scale-100 opacity-100"
        leave-to-class="scale-0 opacity-0"
      >
        <p
          v-if="tableData.text"
          class="px-2 py-1 bg-sky-300 rounded-xl scl shadow-lg shadow-blue-600 antialiased
           text-white hover:transition-colors duration-200 hover:animate-bounce tracking-wide 
           uppercase font-bold transition ease-in-out"
        >
          HaHaHaHaHaHaHaHaaaa....
        </p>
      </transition>
