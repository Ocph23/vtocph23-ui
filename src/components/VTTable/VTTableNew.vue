<template>
  <div class="p-0">
    ...
    <div
      :class="[
        'border-[1.5px] border-gray-400 overflow-x-auto custom-scrollbar',
        isLargeScreen ? 'rounded-lg' : 'rounded-[.80rem]'
      ]"
    >
      <table
        :class="[
          classProp,
          'min-w-full border-collapse bg-transparent text-gray-800 dark:text-gray-200 rounded-t-lg overflow-hidden'
        ]"
      >
        ...
        <thead v-else-if="isLargeScreen">
          <tr>
            <Th
              v-for="column in columnsProp.filter((x) => !x.hiddenColumn)"
              :key="column.name"
              :title="column.title"
              :column-key="column.propName || ''"
              :columns="columnsProp"
              :header-class="column.headerClass"
              :header-position="column.headerPosition"
              @sort="onSort"
            />
          </tr>
        </thead>
        <tbody v-if="isLargeScreen">
          ...
          >
            <td
              v-for="column in headerColumns.filter((x) => !x.hiddenColumn)"
              :key="column.name"
              :class="[
                borderedProp ? ' border-[1px]' : '',
                column.rowClass,
                'px-4 py-2 border-r-0 border-b-[1px] border-gray-100 text-nowrap'
              ]"
            >
            ...
            <VTAccordionPanel v-for="row in sortedData" :key="row.name">
              ...
              <VTAccordionContent class="relative">
                ...
                >
                  ...
                  <VTLabelItem
                    v-if="col.type !== 'Custome' && col.type !== 'Tanggal'"
                    :labelText="col.title"
                    :value="row[col.propName ? col.propName : '']"
                  />
                  <VTLabelItem
                    v-if="col.type === 'Tanggal' && col.propName"
                    :labelText="col.title"
                ...
                <div
                  :class="[
                    'p-3 w-auto rounded-lg text-white absolute pt-0 top-14 right-[.55rem] bg-gray-600',
    ...
    <VTPaginationView
      v-if="showPaginate"
      :page-number="data.paginate.currentPage"
      :total-records="props.source.totalRecords"
      :page-size="data.paginate.pageSize"
...
import { ref, reactive, watch, onMounted, computed, provide, toRef, withDefaults } from 'vue'
...
const updateDisplayedData = () => {
  if (showPaginate.value && props.method === 'All') {
    ...
    if (data.paginate.searchTerm && data.paginate.searchTerm.length >= 3) {
      data.displayedData = searchWithoutPaginate(data.paginate.searchTerm).slice(
        startIndex,
        endIndex
      )
    } else {
...
provide('sort', onSort)
provide('hiddenColumn', {
  updateHiddenColumns,
  tableProps: {
    refresh: updateHiddenColumns,
    columns: columnsProp.value,
    tableName: tableNameProp.value
  }
})
provide('hiddenColumnx', {
  tableProps: { bordered: borderedProp.value, columns: headerColumns.value }
})
