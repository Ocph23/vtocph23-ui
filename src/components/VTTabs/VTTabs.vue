<template>
  <div :class="divClasses">
    <ul :class="ulClasses">
      <fwb-tab-pane v-for="(item, id) in tabsChildren" :key="id" :active="modelValueRef === item.props?.name"
        :disabled="item.props?.disabled" :name="item.props?.name" :title="item.props?.title" @click="emitClick" />
    </ul>
  </div>
  <div v-bind="$attrs">
    <slot />
