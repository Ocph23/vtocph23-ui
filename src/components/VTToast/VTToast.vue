<template>
  <div v-if="visible" id="toast-default" :class="wrapperClasses" role="alert">
    <flowbite-themable-child v-if="type !== 'empty' || $slots.icon" :apply="['background', 'text']"
      ...
      <svg v-else-if="type === 'success'" aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
        xmlns="http://www.w3.org/2000/svg">
        <path clip-rule="evenodd"
          d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
          fill-rule="evenodd" />
      </svg>
      <svg v-else-if="type === 'danger'" aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
        xmlns="http://www.w3.org/2000/svg">
        <path clip-rule="evenodd"
          d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
          fill-rule="evenodd" />
      </svg>
      <svg v-else-if="type === 'warning'" aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
        xmlns="http://www.w3.org/2000/svg">
        <path clip-rule="evenodd"
    ...
    <div :class="[contentClasses, { 'ml-3': $slots.icon || type !== 'empty' }, 'min-h-24 flex items-center']">
      <slot />
    </div>
    <button v-if="closable" aria-label="Close"
      class="border-none ml-auto -mx-1.5 -my-1.5 text-white hover:text-gray-700 rounded-lg focus:ring-2 focus:ring-gray-700 p-1.5 inline-flex h-8 w-8"
